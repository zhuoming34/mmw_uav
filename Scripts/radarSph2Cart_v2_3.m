...
saveaddr = strcat(addr, 'cartHeat/');
for idx_mat = 1:1500
    filename = strcat('md_1_pm_',num2str(idx_mat),'_radar_heatmap');
    mat = load(strcat(addr,filename,'.mat'));
    heatmap = mat.radar_heatmap; % rho*azimuth*elevation
    ...
    save(strcat(saveaddr,'md_',num2str(1),'_pm_',num2str(idx_mat),'_cart_heatmap','.mat'), 'heatmap_ct');
end
%% plots
show_plots = 0; idx_plots = [1,4];
if show_plots == 1
